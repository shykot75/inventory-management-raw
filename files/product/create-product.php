...
if (isset($_POST['submit'])) {
    ...
    if (empty($errors)) {
        $stmt = $connection->prepare("INSERT INTO products (product_name, sku, category_id, product_price, quantity, status, product_description, product_image) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssidsiss", $product_name, $sku, $category_id, $product_price, $quantity, $status, $product_description, $image_path);

        if ($stmt->execute()) {
            $_SESSION['success'] = "Product created successfully";
        } else {
            $_SESSION['error'] = "Error inserting product into the database";
        }
        ...
        <div class="app-menu flex flex-col w-full h-full">
            <ul class="space-y-2 w-full px-2 mt-2">
                ...
                <li class="relative menu-item">
                    <a href="../admin/dashboard.php" class="parent-item active flex items-center w-full px-3 py-3">
